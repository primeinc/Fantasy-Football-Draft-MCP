...
Nothing here is a fitted score. Candidates are ordered by ESPN's projection for
the claim week, then by last week's actual points, the ordering
`waivers.RANK_BY` measured ahead of role change. The drop is the bench player
with the lowest claim-week projection whose played-week game is final: a bench
player still to play is held, because his result is evidence the drop decision
has not seen. A starter ESPN has slotted is never offered as a drop.
"""
...
BASIS = {
    ...
    "drops": ("ESPN BENCH slot only; lowest ESPN claim-week projection first; a player whose "
              "played-week game is not final on ESPN's scoreboard is held; undroppable players "
              "and a player whose loss leaves his position short are not offered"),
}
...
def pending_teams(games: list[dict], scoreboard_week: int | None,
    ...
    """Teams whose `played_week` game is not final, or None when the scoreboard
    shows a different week and the question cannot be answered from it."""
...
def drop_options(mine: list[dict], slots: dict, required: dict[str, int], bye_teams: set[str],
    ...
    for row in mine:
        ...
        hold = (f"his {row['pro_team']} game this week is not final"
                if pending is not None and row["pro_team"] in pending else None)
        options.append({**_evidence(row), "hold": hold})
    options.sort(key=lambda r: (r["hold"] is not None,
                                r["claim_week_proj"] if r["claim_week_proj"] is not None else 0.0,
                                r["last_week_points"] if r["last_week_points"] is not None else 0.0))
    return options, excluded


def candidates(merged: list[dict], position: str, limit: int) -> list[dict]:
    """Acquirable players at `position` who are not out, best first."""
    ...
    rows.sort(key=lambda r: (r["claim_week_proj"] is None, -(r["claim_week_proj"] or 0.0),
                             -(r["last_week_points"] or 0.0)))
    return [_evidence(r) for r in rows[:max(1, limit)]]
...
def plan(need_rows: list[dict], drops: list[dict], merged: list[dict], limit: int,
    ...
    for k, need in enumerate(need_rows, start=drops_taken):
        ...
        for rank, cand in enumerate(candidates(merged, need["position"], limit)):
            claims.append({
                "order": len(claims) + 1, "need": need["position"],
                "add": cand["player"], "drop": None if drop is None else drop["player"],
                "fallback_for": None if rank == 0 else claims[-rank]["add"],
                "add_evidence": cand,
...
def upgrades(merged: list[dict], drops: list[dict], limit: int) -> list[dict]:
    """Optional: acquirable players projected above the cheapest usable drop."""
    usable = [d for d in drops if d["hold"] is None and d["claim_week_proj"] is not None]
    if not usable:
        return []
    weakest = usable[0]
    rows = [r for r in acquirable(merged)
            if r["claim_week_proj"] is not None and r["injury_status"] not in OUT_STATUSES
            and r["claim_week_proj"] > weakest["claim_week_proj"]]
    rows.sort(key=lambda r: -r["claim_week_proj"])
    return [{"add": r["player"], "position": r["position"], "drop": weakest["player"],
             "margin": round(r["claim_week_proj"] - weakest["claim_week_proj"], 2),
             "add_evidence": _evidence(r)} for r in rows[:max(1, limit)]]
